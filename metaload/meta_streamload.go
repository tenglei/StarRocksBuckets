...
// MetaStreamload
// 数据导入到表
func MetaStreamload(conn *util.SrAvgs, sitem *[]MetaInfo, schema string) {
	...
	response, err := Client.SetRedirectPolicy(resty.RedirectPolicyFunc(func(req *http.Request, via []*http.Request) error {
		...
		SetHeaders(map[string]string{
			"label":             fmt.Sprintf("%s_%s_%d", filepath.Base(os.Args[0]), time.Now().Format("2006_01_02_15_04_05"), time.Now().UnixMilli()), /*label*/
		...
		}).SetBasicAuth(conn.User, conn.Pass).
		SetBody(marshal).
		Put(fmt.Sprintf("http://%s:%d/api/%s/%s/_stream_load", conn.Host, 8030, tb[0], tb[1]))
	if err != nil {
		util.Loggrs.Error(err.Error())
		return
	}
