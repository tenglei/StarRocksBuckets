...
本版本重构了StarRocks集群连接方式，从硬编码配置改为交互式输入：

**新特性：**
- ✅ 运行时交互式输入连接参数（FE地址、端口、用户名、密码）
- ✅ 密码隐藏输入，提升安全性
- ✅ 连接参数自动验证
- ✅ 移除对外部MySQL配置库的依赖
- ✅ 一个二进制文件适配多环境
...
请输入FE地址 [默认: localhost]: localhost
请输入FE端口 [默认: 9030]: 9030
请输入用户名 [默认: root，直接回车使用默认]: root
请输入密码（输入时不显示）: 
...
**重要：以下命令中的 `-s <app>` 参数已不再需要，请直接忽略。**

1.`setbuckets -t <schema.tablename>`
